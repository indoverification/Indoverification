...
async function issueOtp(key, emailAddress, purpose) {
  ...
  const now = new Date();
  if (prev.rows[0] && now.getTime() - new Date(prev.rows[0].sent_at).getTime() < OTP_RESEND_MS) {
...
async function verifyOtp(key, value) {
  ...
  if (hash(String(value || '')) !== item.code_hash) {
...
function publicUser(user) { return { id: user.id, email: user.email, name: user.name, active: user.active !== false }; }
...
async function main(req, res) {
  ...
  try {
    ...
    const status = /Authorization|session/.test(message) ? 401 : /already exists/.test(message) ? 409 : /OTP|account|password/i.test(message) ? 400 : 500;
