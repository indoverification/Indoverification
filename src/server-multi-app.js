...
function emailShell(appId, { eyebrow, body, welcome = false }) {
  ...
  return `<!doctype html><html><body style="margin:0;padding:24px 12px;background:#f5f8fc;color:#152033;font-family:Arial,Helvetica,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:620px;background:#fff;border:1px solid #dce5f0;border-radius:16px;overflow:hidden"><tr><td align="center" style="padding:28px 20px 22px;border-bottom:1px solid #e7edf5">${logoHtml(brand)}${verificationNote}<div style="margin-top:8px;color:#6b778c;font-size:12px">${escapeHtml(eyebrow)}</div></td></tr><tr><td style="padding:30px 26px">${body}</td></tr><tr><td align="center" style="padding:18px 20px;background:#f8fafc;border-top:1px solid #e7edf5;color:#7b8799;font-size:12px;line-height:1.7">Automated email from <strong style="color:${color}">${name}</strong>${welcome ? '' : '. Please do not share your OTP.'}</td></tr></table></td></tr></table></body></html>`;
}
...
async function consumeSignupWelcomeToken(appId, token, emailAddress, name) {
  ...
  if (Date.now() > new Date(item.expires_at).getTime()) {
    await pool.query('DELETE FROM signup_welcome_tokens WHERE token_hash=$1', [tokenHash]);
...
async function main(req, res) {
  ...
  try {
    if (url.pathname === '/health' && req.method === 'GET') {
      return sendJson(res, 200, { ok: true, ready: dbReady, service: 'IndoVerification', role: 'multi-app OTP service', apps: listApps().map(({ id, name }) => ({ id, name })), emailConfigured: true, time: new Date().toISOString() });
    }

    if (!dbReady) return sendJson(res, 503, { ok: false, error: 'Service is starting. Please retry in a moment.' });
...
server.listen(PORT, HOST, () => {
  console.log(`IndoVerification multi-app service listening on ${HOST}:${PORT}`);
  initDb().then(() => {
    dbReady = true;
    console.log('IndoVerification database ready');
  }).catch((error) => {
    dbReady = false;
